...
All entity tables are managed inside [src/db/schema](file:///c:/Users/Aman/Desktop/apex-template/server/src/db/schema) using Drizzle ORM syntax. Relationships, cascading deletions, indexes, and primary/foreign key attributes are fully typed.
...
1. **Title Generation:** Conducted in the background using a Mistral model helper [generateChatTitle](file:///c:/Users/Aman/Desktop/apex-template/server/src/services/ai/response.ai.service.js) restricting output to plain strings.
...
The server integrates a dual-mode dispatch mechanism inside [`src/services/mail/mail.service.js`](file:///c:/Users/Aman/Desktop/apex-template/server/src/services/mail/mail.service.js):
...
The system manages secure OTP verification workflows backed by Redis using [`src/utils/otp.utils.js`](file:///c:/Users/Aman/Desktop/apex-template/server/src/utils/otp.utils.js).
...
Controllers should always return standard responses using [`src/utils/response.utlis.js`](file:///c:/Users/Aman/Desktop/apex-template/server/src/utils/response.utlis.js) to ensure consistent payloads across the entire client-server boundary.
...
Image, PDF, or document uploads are handled using [`src/services/image.service.js`](file:///c:/Users/Aman/Desktop/apex-template/server/src/services/image.service.js) backed by the ImageKit Node.js SDK.
...
Use the ioredis instance inside [`src/config/cache.config.js`](file:///c:/Users/Aman/Desktop/apex-template/server/src/config/cache.config.js) to store custom cache values, manage session blacklist tokens, or track rate limits.
...
The system provides a utility to generate cryptographically secure temporary passwords (e.g., for recovery, onboarding, or automated provisioning) using [`src/utils/password.utils.js`](file:///c:/Users/Aman/Desktop/apex-template/server/src/utils/password.utils.js).
