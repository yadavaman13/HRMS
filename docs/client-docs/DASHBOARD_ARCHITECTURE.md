...
## 7. Pluggable Infrastructure Services (e.g., Maps Service)

### A. Architecture Overview

For features that rely on dynamic third-party providers (such as maps, payment gateways, or text-to-speech), the codebase uses a **Pluggable Infrastructure Service** pattern. This isolates vendor-specific SDK integrations behind a clean adapter interface, making the UI layer completely provider-agnostic.

```text
  ┌────────────────────────────────────────────────────────┐
  │                        UI Layer                        │ (MapView, MapMarker, PlaceAutocomplete)
  └──────────────────────────┬─────────────────────────────┘
                             │ (Consumes via Context Hook)
                             ▼
  ┌────────────────────────────────────────────────────────┐
  │                 useMaps() Context Hook                 │
  └──────────────────────────┬─────────────────────────────┘
                             │
            ┌────────────────┴────────────────┐
            ▼ (google)                        ▼ (osm)
  ...
  │  Infrastructure  │               │  StreetMap Infra │
  └──────────────────┘               └──────────────────┘
```

### B. Map Provider Factory Pattern

The map system is instantiated at the root wrapper level using the `MapProviderFactory` component. It inspects the `VITE_MAP_PROVIDER` environment variable and dynamically binds the context to the correct vendor adapters:

- **Google Maps**: Loads the Google Maps JavaScript SDK, exposes Places autocomplete input services, Geocoding adapter, and Directions routing services.
- **OpenStreetMap (OSM)**: Employs Leaflet mapping engine. Uses Nominatim API for geocoding and OSRM API for directions routing (fully open-source and free, avoiding key requirements).

### C. Folder Structure (`src/infrastructure/maps/`)

- `components/`: UI wrappers like `<MapView />`, `<MapMarker />`, `<MapPolyline />`, and `<PlaceAutocomplete />` which automatically switch rendering engines based on the active provider.
- `context/`: Defines the `MapsContext` state interface containing provider keys, initialization status (`loading | ready | error`), and adapter functions.
- `providers/`: Under-the-hood integrations for Google Maps and OpenStreetMap (OSM).
- `capabilities.js`: Static flag definitions outlining features supported by the active provider (e.g., Google supports place autocomplete searches, whereas OSM does not).
- `config.js`: Validates `import.meta.env` keys and configures fallback settings.
