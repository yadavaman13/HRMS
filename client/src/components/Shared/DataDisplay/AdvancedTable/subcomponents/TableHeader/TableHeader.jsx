                ...
                {effectiveColumns.map((col) => {
                    ...
                    if (isCollapsed) {
                        return (
                            <th
                                key={col.key}
                                className={`advanced-table-header-cell is-collapsed-column ${col.key}-cell ${col.className || ''}`}
                                style={{ width: '52px', minWidth: '52px', maxWidth: '52px' }}
                                onClick={() => toggleCollapse && toggleCollapse(col.key)}
                            >
                                <Tooltip
                                    content="Expand"
                                    position="top"
                                    className="header-collapse-tooltip"
                                    usePortal
                                >
                                    <div className="header-cell-content collapsed-inner">
                                        <span className="collapsed-column-title">{col.label}</span>
                                        <button
                                            type="button"
                                            className="collapse-icon-wrapper is-collapsed"
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                toggleCollapse && toggleCollapse(col.key);
                                            }}
                                            aria-label={`Expand ${col.label} column`}
                                        >
                                            <CollapseIcon size="14" strokeWidth={2.8} />
                                        </button>
                                    </div>
                                </Tooltip>
                            </th>
                    ...
                    return (
                        ...
                        >
                            <div className="header-cell-content">
                                ...
                                {showColumnToggleIcon && toggleCollapse && isCollapsible && (
                                    <Tooltip
                                        content="Collapse"
                                        position="top"
                                        className="header-collapse-tooltip"
