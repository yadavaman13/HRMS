 ...
 *  - columns        {Array}     Column definitions from parent DataView
 *  - cardTitleKey   {string}    Row key used as the card heading
 *  - cardSubtitleKey{string}    Row key used as the card subheading
 *  - cardImageKey   {string}    Row key with image/avatar URL (uses CircularAvatar)
...
function GridCard({
    ...
    // Auto-derive avatar initials from title
    const avatarInitial = titleVal
        ? titleVal
              .split(' ')
              .map((w) => w[0])
              .slice(0, 2)
